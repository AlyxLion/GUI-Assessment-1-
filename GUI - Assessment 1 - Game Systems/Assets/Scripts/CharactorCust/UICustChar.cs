...
public class UICustChar : Stats
{
    ...
    private string[] pointName = new string[6] { "Health", "Health Regen", "Speed", "Stamina", "Mana", "Mana Regen" };
...

    {
        ...
        {
            pointSystem[i].stat.name = pointName[i];
        }
        ChooseClass(0);

        //make the drop down display stuff
        var classDropDown = GameObject.Find("Dropdown - Class").GetComponent<Dropdown>(); //grab that dropdown and make it work 
        classDropDown.options.Clear(); //clear options just in case
        //this call fro the aray 
        CharacterClass[] classes = (CharacterClass[])System.Enum.GetValues(typeof(CharacterClass));
    ...
    void SaveCharacter()
    {
        ...
        for (int i = 0; i < pointSystem.Length; i++)
        {
            PlayerPrefs.SetInt(pointSystem[i].nameDisplay.text, (pointSystem[i].stat.value + pointSystem[i].stat.tempValue));
        }
    }
}
